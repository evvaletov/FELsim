...
# Based on UHM_beamline_opt.py with bunch spread reduced from 2 ps to 0.5 ps.
# Energy spread scaled by longitudinal emittance conservation: sigma_t * sigma_E ~ const,
# so 4x shorter bunch -> 4x larger energy spread (0.5% -> 2.0%).
# Chirp parameter h scaled proportionally (4x) for the steeper energy-time correlation.
Energy = 40  # MeV
f = 2856e6  # Hz
bunch_spread = 0.5  # ps (was 2 ps in baseline study)
energy_std_percent = 2.0  # % — scaled from 0.5% by longitudinal emittance conservation
h = 20e9  # 1/s — scaled from 5e9 for 4x bunch compression
...
# Optimization - Fifth Chromacity Quad + Final Triplet (joint, 4 variables)
# Joint optimization of chromaticity quad 5 and the final triplet provides the
# extra degree of freedom needed for simultaneous alpha and beta matching in
# both planes at the undulator entrance.  This resolves the overconstrained
# 3-variable problem that limited independent stage-by-stage optimization.
