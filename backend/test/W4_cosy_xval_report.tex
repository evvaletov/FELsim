...
\begin{table}[h]
\centering
\caption{Optimised quadrupole currents: FELsim, COSY FR\,0,
COSY FR\,1 (warm-started from FR\,0), and RF-Track (Stage~11 only,
warm-started from FELsim).}
...
\paragraph{Key enablers for successful optimisation.}
Several design choices were critical for making the 11-stage optimisation
converge reliably:
\begin{enumerate}
  \item \textbf{Sequential decomposition into physically motivated stages.}
    The 23-quadrupole problem is decomposed into 11 stages, each
    corresponding to a distinct optical section (doublet, triplet,
    chromaticity section).  Each stage optimises 1--4 variables against
    local objectives, avoiding the curse of dimensionality that would
    afflict a monolithic 23-variable optimisation.

  \item \textbf{Joint 4-variable final stage.}
    With the asymmetric undulator Twiss targets from the paper ($\beta_x
    \neq \beta_y$, $\alpha_x \neq 0$), the final triplet alone (3~variables)
    is overconstrained against 4~Twiss objectives.  Promoting chromaticity
    quad~5 (element~87) to a 4th joint variable resolves this: MSE drops
    from $8.6 \times 10^{-2}$ (3-var) to $4.3 \times 10^{-9}$ (4-var).
    A differential-evolution global search confirms this is the global
    optimum.

  \item \textbf{Correct initial Twiss from the beam distribution.}
    The initial beta function must be computed from the beam statistics,
    $\beta_0 = \sigma_x^2 / \varepsilon \approx \SI{6.34}{m}$, not the
    common default $\beta_0 = \SI{1}{m}$.  Using the wrong initial Twiss
    propagates through the entire lattice and produces completely incorrect
    transport (e.g., $\alpha_x = -2.8$ instead of $\approx 0$ after
    Stage~1).

  \item \textbf{Combined MSE objective for gradient-based optimisation.}
    COSY's FIT with individual objectives resets all variables when
    \emph{any} objective fails to converge.  A single combined MSE
    objective, preserving DA derivatives (no \texttt{CONS()} calls),
    allows the gradient-based algorithm to converge monotonically.

  \item \textbf{Warm-starting across physics models.}
    Running FR\,1 cold from default initial guesses fails
    (MSE~$\sim 0.2$); warm-starting from the converged FR\,0 solution
    reduces this to MSE~$\sim 8 \times 10^{-8}$ with only minor current
    adjustments ($\Delta I_\mathrm{max} = \SI{0.76}{A}$).
\end{enumerate}
