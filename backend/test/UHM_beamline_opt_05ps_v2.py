...
#
# Revision of UHM_beamline_opt_05ps.py following Weinberg, Fisher & Li
# (arXiv:2510.14061v1).
#
# Key differences from the previous 0.5 ps study:
#   1. Energy spread and chirp are UNCHANGED from the 2 ps baseline (0.5%, 5e9 /s).
#      The paper isolates the effect of bunch shortening by keeping all other beam
#      parameters the same as the nominal case (§III, p.5).
#   2. Undulator Twiss targets use the paper's asymmetric matching (Table I):
#        Horizontal:  beta_x = 1.4 m,   alpha_x = 0.47  (radiation mode matching)
#        Vertical:    beta_y = 0.24 m,   alpha_y = 0     (natural undulator focusing)
#      The horizontal beta is set by matching the electron beam size to the radiation
#      spot size: sigma_x = sigma_r = sqrt(lambda_r * Z_R / (4*pi)), with the waist
#      at the undulator center giving alpha_x > 0 at the entrance.
#
...
# ── Beam parameters ──────────────────────────────────────────────────────────
# Unchanged from the 2 ps baseline except for bunch_spread.
# Per arXiv:2510.14061v1 §III p.5: "we only change the bunch length and keep
# the other machine and beam parameters the same as the nominal case."
Energy = 40             # MeV
f = 2856e6              # Hz
bunch_spread = 0.5      # ps  (nominal: 2 ps)
energy_std_percent = 0.5  # %  — unchanged from baseline
...
# sigma_x = sigma_r = sqrt(lambda_r * Z_R / (4*pi)),  beta_x = sigma_r^2 / epsilon_x.
...
# Consistency check: waist should be at undulator center (L_u/2 from entrance)
